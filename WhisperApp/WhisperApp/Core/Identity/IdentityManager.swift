...
class CoreDataIdentityManager: IdentityManager {
    ...
    func createIdentity(name: String) throws -> Identity {
        ...
        entity.expiresAt = Calendar.current.date(byAdding: .year, value: 1, to: newIdentity.createdAt)
    ...
    func listIdentities() -> [Identity] {
        ...
        request.sortDescriptors = [NSSortDescriptor(keyPath: \IdentityEntity.createdAt, ascending: false)]
    ...
    func rotateActiveIdentity() throws -> Identity {
        ...
        // Create new identity with incremented version
        let newIdentity = try createIdentity(name: currentActive.name)
    ...
    func getIdentitiesNeedingRotationWarning() -> [Identity] {
        ...
        return identities.filter { identity in
            ...
            return timeUntilRecommendedRotation <= rotationWarningThreshold && timeUntilRecommendedRotation > 0
        }
    }
