...
class WhisperEnvelopeProcessor: EnvelopeProcessor {
    ...
    func createEnvelope(
        ...
        print("🔐 WHISPER ENCRYPTION DEBUG LOG - START")
        print(String(repeating: "=", count: 80))
        print("🔐 ENCRYPT DEBUG: Building canonical context...")
        print("🔐 ENCRYPT DEBUG: Sender identity: \(senderIdentity.name)")
        print(
            "🔐 ENCRYPT DEBUG: Sender fingerprint: \(senderIdentity.fingerprint.prefix(8).base64EncodedString())"
        )
        print("🔐 ENCRYPT DEBUG: Require signature: \(requireSignature)")
        print("🔐 ENCRYPT DEBUG: Flags: \(flags)")
        print("🔐 ENCRYPT DEBUG: RKID: \(rkid.base64EncodedString())")

        let recipientFingerprint = generateRecipientFingerprint(recipientPublic)
        print(
            "🔐 ENCRYPT DEBUG: Recipient fingerprint: \(recipientFingerprint.prefix(8).base64EncodedString())"
        )
        ...
        print("🔐 ENCRYPT DEBUG: ✅ Canonical context built successfully")
        print(String(repeating: "=", count: 80))
        ...
        if requireSignature, let ed25519KeyPair = senderIdentity.ed25519KeyPair {
            ...
            signature = try cryptoEngine.sign(
                data: signatureData, privateKey: ed25519KeyPair.privateKey)
        }

        // Build envelope string
        let envelopeString = try buildEnvelopeString(
            version: "v1.c20p",
            rkid: rkid,
            flags: flags,
            epk: ephemeralPublic,
        ...
        print("🔐 ENCRYPT DEBUG: 🎉 Encryption completed successfully!")
        print("🔐 ENCRYPT DEBUG: Final envelope length: \(envelopeString.count)")
        print("🔐 ENCRYPT DEBUG: Final envelope prefix: \(String(envelopeString.prefix(50)))")
        print(String(repeating: "=", count: 80))
        print("🔐 WHISPER ENCRYPTION DEBUG LOG - SUCCESS")
        print(String(repeating: "=", count: 80) + "\n")
    ...
    func parseEnvelope(_ envelope: String) throws -> EnvelopeComponents {
        print("🔍 ENVELOPE_PROCESSOR: parseEnvelope called")
        print("🔍 ENVELOPE_PROCESSOR: Envelope length: \(envelope.count)")
        print("🔍 ENVELOPE_PROCESSOR: Envelope: \(envelope)")
        
        // Validate envelope prefix
        guard envelope.hasPrefix("whisper1:") else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Missing whisper1: prefix")
            throw EnvelopeError.invalidFormat
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ Has whisper1: prefix")
        ...
        print("🔍 ENVELOPE_PROCESSOR: Envelope body: \(envelopeBody)")
        print("🔍 ENVELOPE_PROCESSOR: All components count: \(allComponents.count)")
        print("🔍 ENVELOPE_PROCESSOR: All components: \(allComponents)")
        ...
        guard allComponents.count == 9 || allComponents.count == 10 else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid component count: \(allComponents.count), expected 9 or 10")
            throw EnvelopeError.invalidFormat
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ Valid component count: \(allComponents.count)")

        // Reconstruct version from first two components
        let version = String(allComponents[0]) + "." + String(allComponents[1])
        print("🔍 ENVELOPE_PROCESSOR: Reconstructed version: \(version)")
        guard version == "v1.c20p" else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Unsupported version: \(version), expected v1.c20p")
            throw EnvelopeError.unsupportedAlgorithm(version)
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ Valid version: \(version)")

        // Get the actual envelope components (skip the version parts)
        let components = Array(allComponents.dropFirst(2))
        print("🔍 ENVELOPE_PROCESSOR: Envelope components (after version): \(components)")
        print("🔍 ENVELOPE_PROCESSOR: Component count (after version): \(components.count)")

        // Parse and decode components (now 0-based since we dropped version parts)
        print("🔍 ENVELOPE_PROCESSOR: Decoding RKID...")
        let rkid = try base64URLEncoder.decode(String(components[0]))
        guard rkid.count == 8 else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid RKID length: \(rkid.count), expected 8")
            throw EnvelopeError.invalidRkidLength
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ RKID decoded: \(rkid.base64EncodedString())")

        print("🔍 ENVELOPE_PROCESSOR: Decoding flags...")
        let flagsData = try base64URLEncoder.decode(String(components[1]))
        guard flagsData.count == 1 else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid flags length: \(flagsData.count), expected 1")
            throw EnvelopeError.invalidFlagsLength
        }
        let flags = flagsData[0]
        print("🔍 ENVELOPE_PROCESSOR: ✅ Flags decoded: \(flags)")

        print("🔍 ENVELOPE_PROCESSOR: Decoding EPK...")
        let epk = try base64URLEncoder.decode(String(components[2]))
        guard epk.count == 32 else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid EPK length: \(epk.count), expected 32")
            throw EnvelopeError.invalidEpkLength
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ EPK decoded: \(epk.prefix(8).base64EncodedString())...")

        print("🔍 ENVELOPE_PROCESSOR: Decoding salt...")
        let salt = try base64URLEncoder.decode(String(components[3]))
        guard salt.count == 16 else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid salt length: \(salt.count), expected 16")
            throw EnvelopeError.invalidSaltLength
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ Salt decoded: \(salt.prefix(8).base64EncodedString())...")

        print("🔍 ENVELOPE_PROCESSOR: Decoding msgid...")
        let msgid = try base64URLEncoder.decode(String(components[4]))
        guard msgid.count == 16 else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid msgid length: \(msgid.count), expected 16")
            throw EnvelopeError.invalidMsgidLength
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ Msgid decoded: \(msgid.prefix(8).base64EncodedString())...")

        print("🔍 ENVELOPE_PROCESSOR: Decoding timestamp...")
        let timestampData = try base64URLEncoder.decode(String(components[5]))
        guard timestampData.count == 8 else {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid timestamp length: \(timestampData.count), expected 8")
            throw EnvelopeError.invalidTimestampLength
        }
        let timestamp = timestampData.withUnsafeBytes { $0.load(as: Int64.self).bigEndian }
        print("🔍 ENVELOPE_PROCESSOR: ✅ Timestamp decoded: \(timestamp)")

        print("🔍 ENVELOPE_PROCESSOR: Decoding ciphertext...")
        let ciphertext = try base64URLEncoder.decode(String(components[6]))
        print("🔍 ENVELOPE_PROCESSOR: ✅ Ciphertext decoded: \(ciphertext.count) bytes")

        // Parse signature if present
        var signature: Data? = nil
        if components.count == 8 {  // 8 components means signature is present (7 + signature)
            print("🔍 ENVELOPE_PROCESSOR: Decoding signature...")
            signature = try base64URLEncoder.decode(String(components[7]))
            guard signature?.count == 64 else {
                print("🔍 ENVELOPE_PROCESSOR: ❌ Invalid signature length: \(signature?.count ?? 0), expected 64")
                throw EnvelopeError.invalidSignatureLength
            }
            print("🔍 ENVELOPE_PROCESSOR: ✅ Signature decoded: \(signature!.prefix(8).base64EncodedString())...")
        } else {
            print("🔍 ENVELOPE_PROCESSOR: No signature present")
        }

        // Validate signature flag consistency
        let hasSignature = (flags & 0x01) != 0
        print("🔍 ENVELOPE_PROCESSOR: Signature flag set: \(hasSignature)")
        print("🔍 ENVELOPE_PROCESSOR: Signature data present: \(signature != nil)")
        
        if hasSignature && signature == nil {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Signature flag set but no signature data")
            throw EnvelopeError.signatureFlagMismatch
        }
        if !hasSignature && signature != nil {
            print("🔍 ENVELOPE_PROCESSOR: ❌ Signature data present but flag not set")
            throw EnvelopeError.signatureFlagMismatch
        }
        print("🔍 ENVELOPE_PROCESSOR: ✅ Signature flag consistency validated")

        print("🔍 ENVELOPE_PROCESSOR: ✅ Envelope parsing completed successfully")
