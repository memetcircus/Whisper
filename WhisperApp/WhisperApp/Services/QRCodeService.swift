...
class QRCodeService: ObservableObject {
    ...
    func parseQRCode(_ content: String) throws -> QRCodeContent {
        print("🔍 QR_SERVICE: parseQRCode called")
        print("🔍 QR_SERVICE: Content length: \(content.count)")
        print("🔍 QR_SERVICE: Content prefix (50 chars): \(String(content.prefix(50)))")
        print("🔍 QR_SERVICE: Full content: \(content)")

        if content.hasPrefix("whisper-bundle:") {
            print("🔍 QR_SERVICE: Detected whisper-bundle format")
            return try parsePublicKeyBundle(content)
        } else if content.hasPrefix("whisper1:") {
            print("🔍 QR_SERVICE: Detected whisper1 format")
            return try parseEncryptedMessage(content)
        } else {
            print("🔍 QR_SERVICE: ❌ Unsupported format - no whisper prefix found")
            print("🔍 QR_SERVICE: Content starts with: \(String(content.prefix(20)))")
    ...
    private func generateQRCodeImage(from content: String, type: QRCodeType) throws -> QRCodeResult
    {
        ...
        let sizeWarning =
            contentData.count > Self.maxRecommendedSize
            ? QRCodeSizeWarning(
    ...
    private func parseEncryptedMessage(_ content: String) throws -> QRCodeContent {
        print("🔍 QR_SERVICE: parseEncryptedMessage called")
        print("🔍 QR_SERVICE: Content: \(content)")

        // Validate envelope format - should start with whisper1: and have proper structure
        guard content.hasPrefix("whisper1:") else {
            print("🔍 QR_SERVICE: ❌ Missing whisper1: prefix")
            throw QRCodeError.invalidEnvelopeFormat
        }
        print("🔍 QR_SERVICE: ✅ Has whisper1: prefix")
        ...
        print("🔍 QR_SERVICE: Envelope content: \(envelopeContent)")
        print("🔍 QR_SERVICE: Component count: \(components.count)")
        print("🔍 QR_SERVICE: Components: \(components)")

        // Whisper envelope should have 9 components (unsigned) or 10 components (signed)
        // This accounts for the version "v1.c20p" being split into "v1" and "c20p"
        guard components.count >= 9 && components.count <= 10 else {
            print("🔍 QR_SERVICE: ❌ Invalid component count: \(components.count), expected 9-10")
            throw QRCodeError.invalidEnvelopeFormat
        }
        print("🔍 QR_SERVICE: ✅ Valid component count: \(components.count)")

        // Validate that each component is valid base64url
        for (index, component) in components.enumerated() {
            guard !component.isEmpty else {
                print("🔍 QR_SERVICE: ❌ Empty component at index \(index)")
                throw QRCodeError.invalidEnvelopeFormat
            }
            print("🔍 QR_SERVICE: Component \(index): '\(component)' (length: \(component.count))")
        }

        print("🔍 QR_SERVICE: ✅ All components valid, returning encrypted message")
