...
class CoreDataContactManager: ContactManager {
    ...
    func searchContacts(query: String) -> [Contact] {
        let request: NSFetchRequest<ContactEntity> = ContactEntity.fetchRequest()
        request.predicate = NSPredicate(format: "displayName CONTAINS[cd] %@ OR note CONTAINS[cd] %@", query, query)
        request.sortDescriptors = [NSSortDescriptor(key: "displayName", ascending: true)]
    ...
    func handleKeyRotation(for contact: Contact, newX25519Key: Data, newEd25519Key: Data? = nil) throws {
        ...
        finalContact.trustLevel = .unverified // Reset trust level on key rotation
        finalContact.keyHistory.append(oldKeyHistory)
...
extension KeyHistoryEntry {
    init(id: UUID, keyVersion: Int, x25519PublicKey: Data, ed25519PublicKey: Data?, fingerprint: Data, createdAt: Date) {
        self.id = id
